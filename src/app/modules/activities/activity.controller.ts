...
const allActivity = catchAsync(async (req, res, next) => {
  ...
  // totale items
  const count = await ActivityModel.find(filter).countDocuments();
  // user find
  const users = await ActivityModel.find(filter)
    .limit(limit)
    .skip((page - 1) * limit);
...
const updateActivity = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!req.file) {
    ...
    const updatedData = await ActivityModel.findByIdAndUpdate(id, { name, address, price, goOn, cityId, nearHospitalId }, { new: true });
    if (!updatedData) {
...
const deleteActivity = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const deletedData = await ActivityModel.findByIdAndDelete(id);
