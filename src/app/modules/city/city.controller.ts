...
const allCity = catchAsync(async (req, res, next) => {
  ...
  // totale items
  const count = await CityModel.find(filter).countDocuments();
  // user find
  const users = await CityModel.find(filter)
    .limit(limit)
    .skip((page - 1) * limit);
...
const updateCity = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!req.file) {
    ...
    const updatedData = await CityModel.findByIdAndUpdate(id, { name, temperature, countryId }, { new: true });
    if (!updatedData) {
  ...
  // Find the document by ID and update it
  const updatedData = await CityModel.findByIdAndUpdate(id, { name, temperature, countryId, img: NewFilePath }, { new: true });
  if (!updatedData) {
    throw new ApiErrors(404, 'Data not found')
  }
  sendResponse(res, {
...
// ! ====== Update city =======
const deleteCity = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const id = req.query.id as string;
  // check id 
  if (!id) {
    throw new ApiErrors(400, 'id is required')
  ...
  const deletedData = await CityModel.findByIdAndDelete(id);
