...
// ! ====== Get  =======
const allCountry = catchAsync(async (req, res, next) => {
    ...
    // totale items
    const count = await CountryModel.find(filter).countDocuments();
    // user find
    const users = await CountryModel.find(filter)
      .limit(limit)
      .skip((page - 1) * limit);
...
// ! ====== create  =======
const createCountry = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const imgPath: string = `/uploads/country/${filename}`;
...
// ! ====== Update  =======
const updateCountry = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!req.file) {
        ...
        const updatedData = await CountryModel.findByIdAndUpdate(id, { name, address, price, goOn, cityId, nearHospitalId }, { new: true });
        if (!updatedData) {
    ...
    // new file path
    const NewFilePath: String = "/uploads/country/" + newFilename;
    // old data get from db
    const oldData = await CountryModel.findById(id);
    if (!oldData) {
        throw new ApiErrors(404, 'Data not found')
    }
    // Delete old file 
    ...
    // Find the document by ID and update it
    const updatedData = await CountryModel.findByIdAndUpdate(id, { name, address, price, goOn, cityId, nearHospitalId, img: NewFilePath }, { new: true });
    if (!updatedData) {
        throw new ApiErrors(404, 'Data not found')
    }
    sendResponse(res, {
...
// ! ====== Update  =======
const deleteCountry = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const id = req.query.id as string;
    // check id 
    if (!id) {
        throw new ApiErrors(400, 'id is required')
