...
// ! ====== Get  =======
const allHotel = catchAsync(async (req, res, next) => {
  ...
  // totale items
  const count = await HotelModel.find(filter).countDocuments();
  // user find
  const users = await HotelModel.find(filter)
    .limit(limit)
    .skip((page - 1) * limit);
...
const updateHotel = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!req.file) {
    ...
    const updatedData = await HotelModel.findByIdAndUpdate(id, { name, address, price, goOn, cityId, nearHospitalId }, { new: true });
    if (!updatedData) {
  ...
  // Find the document by ID and update it
  const updatedData = await HotelModel.findByIdAndUpdate(id, { name, address, price, goOn, cityId, nearHospitalId, img: NewFilePath }, { new: true });
  if (!updatedData) {
    throw new ApiErrors(404, 'Data not found')
  }
  sendResponse(res, {
...
// ! ====== delete  =======
const deleteHotel = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const id = req.query.id as string;
  // check id 
  if (!id) {
    throw new ApiErrors(400, 'id is required')
