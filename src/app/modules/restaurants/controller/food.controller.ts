...
// ! ====== Get  =======
const allFood = catchAsync(async (req, res, next) => {
    ...
    // user find
    const users = await FoodModel.find(filter)
      .limit(limit)
      .skip((page - 1) * limit);
...
// ! ====== create  =======
const createFood = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
const updateFood = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!req.file) {
        ...
        const updatedData = await FoodModel.findByIdAndUpdate(id, { name, address, price, goOn, cityId, nearHospitalId }, { new: true });
        if (!updatedData) {
    ...
    // new file path
    const NewFilePath: String = "/uploads/food/" + newFilename;
    // old data get from db
    const oldData = await FoodModel.findById(id);
    if (!oldData) {
        throw new ApiErrors(404, 'Data not found')
    }
    // Delete old file 
    ...
    // Find the document by ID and update it
    const updatedData = await FoodModel.findByIdAndUpdate(id, { name, address, price, goOn, cityId, nearHospitalId, img: NewFilePath }, { new: true });
    if (!updatedData) {
        throw new ApiErrors(404, 'Data not found')
    }
    sendResponse(res, {
...
const deleteFood = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const deletedData = await FoodModel.findByIdAndDelete(id);
