...
import { SuperMarketModel } from "./super-market.model";


// ! ====== Get  =======
const allSuperMarket = catchAsync(async (req, res, next) => {
  ...
  // totale items
  const count = await SuperMarketModel.find(filter).countDocuments();
  // user find
  const users = await SuperMarketModel.find(filter)
    .limit(limit)
    .skip((page - 1) * limit);
...
// ! ====== create  =======
const createSuperMarket = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const imgPath: string = `/uploads/super-market/${filename}`;
  ...
  try {
    // Add to database
    const data = await SuperMarketModel.create({
      name,
      address,
      distance,
      goOn,
...
// ! ====== Update  =======
const updateSuperMarket = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!req.file) {
    ...
    const updatedData = await SuperMarketModel.findByIdAndUpdate(id, { name, address, distance, goOn, cityId, nearHospitalId }, { new: true });
    if (!updatedData) {
  ...
  // new file path
  const NewFilePath: String = "/uploads/super-market/" + newFilename;
  // old data get from db
  const oldData = await SuperMarketModel.findById(id);
  if (!oldData) {
    throw new ApiErrors(404, 'Data not found')
  }
  // Delete old file 
  ...
  // Find the document by ID and update it
  const updatedData = await SuperMarketModel.findByIdAndUpdate(id, { name, address, distance, goOn, cityId, nearHospitalId, img: NewFilePath }, { new: true });
  if (!updatedData) {
    throw new ApiErrors(404, 'Data not found')
  }
  sendResponse(res, {
...
// ! ====== delete  =======
const deleteSuperMarket = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const id = req.query.id as string;
  // check id 
  if (!id) {
    throw new ApiErrors(400, 'id is required')
  ...
  const deletedData = await SuperMarketModel.findByIdAndDelete(id);
